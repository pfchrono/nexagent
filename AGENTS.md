...
- Use repo labels as role mappings: `bug:triage` for `needs-triage`, `needs-repro` for `needs-info`, `agent-ready` for `ready-for-agent`, `human-ready` for `ready-for-human`, and `wontfix` for `wontfix`. Category labels are `bug` or `enhancement`.
